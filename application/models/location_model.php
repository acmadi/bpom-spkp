...
class Location_model extends CI_Model {
	...
	function select_kotakab($propinsi,$kotakab)
	{
        ...
        foreach($result as $x=>$y){
			$data['kota'].="<option value='".$y['id_kota']."' ".($kotakab==$y['id_kota'] ? "selected":"").">".$y['id_kota']." - ".$y['nama_kota']."</option>";
		}
    ...
    function select_jabatan($subdit,$jabatan){
        $sql = "SELECT * FROM mas_jabatan WHERE id_subdit = '".$subdit."'";
		$query = $this->db->query($sql);
		$result = $query->result_array();
		$data=array();
		$data['jabatan']="<option></option>";
        foreach($result as $x=>$y){
