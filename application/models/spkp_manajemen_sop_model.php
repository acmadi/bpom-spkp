<?php
class Spkp_manajemen_sop_model extends CI_Model {
    ...
    function json_upload(){
        $query = "SELECT @i:=@i+1 AS urut, FROM_UNIXTIME(a.id,'%Y/%m/%d %T') AS waktu,
                ...
                a.keterangan as ket_file, a.filename, a.filesize, a.status, a.ip, b.username FROM spkp_manajemen_sop_file a
    ...
    function get_data_row_file($id){
        $query = $this->db->get_where('spkp_manajemen_sop_file', array('id'=>$id),1);
    ...
    function insert_upload($upload_data){
        ...
        if($this->db->insert('spkp_manajemen_sop_file', $data)){
			return $data['id'];
		}else{
			return false;
		}
    ...
    function update_upload($id,$upload_data=0){
		...
		return $this->db->update('spkp_manajemen_sop_file', $data, array('id'=>$id));
    }
    
    function delete_upload($id){
        $this->db->where('id',$id);
        
        return $this->db->delete('spkp_manajemen_sop_file');
    }
