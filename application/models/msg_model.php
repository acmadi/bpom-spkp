...
class Msg_model extends CI_Model  {
    ...
    function get_users_list($mid){
        $this->db->select("app_users_profile.username,app_users_profile.nama");
        $this->db->where("mid",$mid);
    ...
    function get_userlist(){
        $this->db->select("app_users_profile.username,app_users_profile.nama,app_users_profile.nip");
	...
	function docompose(){
		...
		if ($this->db->insert('app_user_msgs',$data)){
			...
			if ($this->db->insert('app_user_msgs_reply',$reply)){
                ...
                $this->db->insert('app_user_msgs_status',$participant);

				foreach($this->input->post('participant') as $par){
					$participant['mid']=$data['mid'];
					$participant['username']=$par;
					$participant['location']='inbox';
					if($par!=$this->session->userdata('username')){
					   $this->db->insert('app_user_msgs_participant',$participant);
                       $this->db->insert('app_user_msgs_status',$participant);
					} 
				}
	...
	function domove($mid,$location){
		...
		if ($this->db->update('app_user_msgs_participant',$data,$option)){
		    $this->db->update('app_user_msgs_status',$data,$option);
            
            $status['dtime']=time();
            $this->db->update('app_user_msgs_status',$status,$option,1);
	...
	function dodel($mid){
		...
		if ($this->db->delete('app_user_msgs_participant',$option)){
            ...
            */
            ...
            $count = $this->count_status($mid);
            $dlt = $this->cek_msg($mid)->num_rows();
            
            if($count==$dlt){
                $this->db->delete('app_user_msgs_status', array('mid'=>$mid,'location'=>'trash'));
            }
    ...
    function count_users($mid){
        $query = $this->db->get_where('app_user_msgs_participant', array('mid'=>$mid));
        
        return $query->num_rows();
    }
    
    function count_status($mid){
        $query = $this->db->get_where('app_user_msgs_status', array('mid'=>$mid));
        
        return $query->num_rows();
    }
    
    function cek_msg($mid){
        $query = $this->db->query("SELECT a.mid, a.username, a.location, FROM_UNIXTIME(dtime,'%d-%m-%Y %T') AS dtime FROM app_user_msgs_status a
                                    WHERE a.mid = '$mid' AND a.location = 'trash'");
        return $query;
    }	
}