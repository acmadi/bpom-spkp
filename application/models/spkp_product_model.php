<?php
class Spkp_product_model extends CI_Model {
    ...
    function json_judul(){
        $query = "SELECT @i:=@i+1 AS urut, FROM_UNIXTIME(a.id,'%Y/%m/%d %T') AS waktu,
                ...
                a.id AS id_file, a.judul, a.tipe, a.uploader, a.update, 
                a.keterangan as ket_file, a.filename, a.filesize, a.status, a.ip, b.username FROM spkp_product a
    ...
    function get_data_row($id){
        $query = $this->db->get_where('spkp_product', array('id'=>$id),1);
    ...
    function insert_upload($upload_data){
        ...
        if($this->db->insert('spkp_product', $data)){
			return $data['id'];
		}else{
			return false;
		}
    ...
    function update_upload($id,$upload_data=0){
		...
		return $this->db->update('spkp_product', $data, array('id'=>$id));
    }
    
    function delete_upload($id){
        $this->db->where('id',$id);
        return $this->db->delete('spkp_product');
    }
}
?>