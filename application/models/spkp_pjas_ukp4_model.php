<?php
class Spkp_pjas_ukp4_model extends CI_Model {
    ...
    function json_judul($thn){
        $query = "SELECT @i:=@i+1 AS urut, FROM_UNIXTIME(a.id,'%Y/%m/%d %T') AS waktu,
                ...
                a.keterangan as ket_file, a.filename, a.filesize, a.status, a.ip, b.username FROM spkp_pjas_ukp4 a
    ...
    function get_data_row($id){
        $query = $this->db->get_where('spkp_pjas_ukp4', array('id'=>$id),1);
    ...
    function insert_upload($upload_data){
        ...
        if($this->db->insert('spkp_pjas_ukp4', $data)){
			return $data['id'];
		}else{
			return false;
		}
    ...
    function update_upload($id,$upload_data=0){
		...
		return $this->db->update('spkp_pjas_ukp4', $data, array('id'=>$id));
    }
    
    function delete_upload($id){
        $this->db->where('id',$id);
        return $this->db->delete('spkp_pjas_ukp4');
    }
}
?>