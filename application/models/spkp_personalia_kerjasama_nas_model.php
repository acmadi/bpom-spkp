<?php
class Spkp_personalia_kerjasama_nas_model extends CI_Model {
    ...
    function json_departemen_nas(){
        $query = "SELECT @i:=@i+1 AS urut, a.* FROM mas_departemen a WHERE a.jenis = 'Nasional' ORDER BY ket ASC";
    ...
    function json_upload($departemen){
        $query = "SELECT @i:=@i+1 AS urut, FROM_UNIXTIME(b.id,'%Y/%m/%d %T') AS waktu,
                ...
                INNER JOIN spkp_kegiatan_kerjasama_nas_file b ON a.id_departemen = b.departemen                
    ...
    function get_data_row_file($id){
        $query = $this->db->get_where('spkp_kegiatan_kerjasama_nas_file', array('id'=>$id),1);
    ...
    function insert_upload($upload_data){
        ...
        if($this->db->insert('spkp_kegiatan_kerjasama_nas_file', $data)){
			return $data['id'];
		}else{
			return false;
		}
    ...
    function update_upload($id,$upload_data=0){
		...
		return $this->db->update('spkp_kegiatan_kerjasama_nas_file', $data, array('id'=>$id));
    }
    
    function delete_upload($id){
        $this->db->where('id',$id);
        
        return $this->db->delete('spkp_kegiatan_kerjasama_nas_file');
    }
    
    function get_all_departmen_nas(){
        $this->db->order_by('id_departemen','asc');
        $query = $this->db->get_where('mas_departemen', array('jenis'=>'Nasional'));
    ...
    function get_first_departmen(){
		$this->db->where(array('jenis'=>'Nasional'));
		$this->db->order_by('id_departemen','asc');
    ...
    function get_id_file($departemen){
        $query = $this->db->get_where('spkp_kegiatan_kerjasama_nas_file', array('departemen'=>$departemen));
