<?php
class Spkp_personalia_dp3_model extends CI_Model {
	...
	function json_sasaran($id){
		$query="SELECT @i:=@i+1 AS urut,FROM_UNIXTIME(pegawai_dp3.id,'%Y/%m/%d %T') AS waktu,IF(pegawai_dp3.update>1, 
				FROM_UNIXTIME(pegawai_dp3.update,'%Y/%m/%d %T'), 'NULL') AS waktu_update, pegawai_dp3.*, app_users_list.username
				FROM pegawai_dp3 
				INNER JOIN app_users_list ON app_users_list.id=pegawai_dp3.uploader WHERE (pegawai_dp3.uid=".$this->session->userdata("id")." || pegawai_dp3.uploader=".$this->session->userdata("id")." || pegawai_dp3.status=1) AND pegawai_dp3.uid=".$id;
	...
	function doadd($uid,$upload_data){
		...
		if($this->db->insert('pegawai_dp3', $data)){
			return $data['id'];
		}else{
			return false;
		}
	...
	function doedit($id,$upload_data=0){
		...
		return $this->db->update('pegawai_dp3', $data, array('id'=>$id));
	}
