...
$(function(){
	...
	url: "<?php echo base_url(); ?>spkp_pjas_f2/json_rekap/",
		cache: false,
	...
	{		
		...
		columns: [
			...
			{ text: 'Balai', datafield: 'nama_balai', columntype: 'dropdownlist', filtertype: 'list', width: '20%' },
			{ text: 'Nama', datafield: 'penanggungjawab_nama', columntype: 'textbox', filtertype: 'textbox', width: '25%' },
			{ text: 'NIP', datafield: 'penanggungjawab_nip', columntype: 'textbox', filtertype: 'textbox', width: '19%' }
	...
	$('#btn_tambah').click(function(){
		...
		$.get("<?php echo base_url();?>spkp_pjas_f2/add" , function(response) {
	...
	function edit(id){
		window.location.href="<?php echo base_url();?>spkp_pjas_f2/edit/"+id;
	}
	...
	function dodel(urut,id){
		if(confirm("Anda yakin akan menghapus data ("+urut+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/spkp_pjas_f2/dodel/"+id,
...
<div id="popup" style="display:none"><div id="popup_title">F02 Rekapitulasi Kegiatan Fasilitator Keamanan PJAS</div><div id="popup_content">{popup}</div></div>
<div>
