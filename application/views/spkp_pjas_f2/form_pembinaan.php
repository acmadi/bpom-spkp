<script type="text/javascript">
    ...
    function save_add_pembinaan_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_pjas_f2/doadd_pembinaan",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_pembinaan_dialog();
	...
	function save_edit_pembinaan_dialog(content){
	   	$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_pjas_f2/doedit_pembinaan",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_pembinaan_dialog();
...
<div style="display: none;" id="divLoadPembinaan"></div>
    <div id="divFormPembinaan" style="padding:8px">
        <form method="post" id="frmDataPembinaan">
            <div style="background: #F4F4F4;padding: 5px;text-align: right;">
                ...
                <button type="button" onclick="save_{action}_pembinaan_dialog($('#frmDataPembinaan').serialize());">Simpan</button>
            	<button type="reset">Ulang</button>
                <button type="button" onclick="close_pembinaan_dialog();">Batal</button>
            </div>
            </br>
            </br>
