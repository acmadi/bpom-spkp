...
<script>
	...
	$(function(){
	   ...
	   var id= $(this).attr('id').split("__");
			...
			$.get("<?php echo base_url();?>spkp_aksi_dokumentasi/upload/"+id[1] , function(response) {
	...
	$("div[id^='btndelete']").click(function(){
		...
		if(confirm("Anda yakin akan menghapus data ("+id[1]+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/spkp_aksi_dokumentasi/dodelimg/"+id[1],
				success: function(response){
					...
					window.location.href = "<?php echo base_url();?>spkp_aksi_dokumentasi/show/<?php echo $id;?>";
					 }else{
						 $.notific8('Notification', {
						  life: 5000,
						  message: response,
...
<div class="row-fluid">
   <div class="span12">
	   <h3 class="page-title">
		<a href="<?php echo base_url()?>spkp_aksi_dokumentasi">Gallery Kegiatan</a> : : <?php echo $judul;?>
	   </h3>
   </div>
</div>
<?php if($add_permission2==true){ ?>
<form method="POST" form="<?php base_url() ?>spkp_pjas_dokumentasi/{id}" style='float:right'>
...
<div class="row-fluid">	
	...
	<div class="dokumentasi-grid-detail dg-xlarge">		
		...
		<div style="background:#999999;padding:6.5px 5px;position:relative;top:0;float:right;margin-right:2px;cursor:pointer;color:#FFFFFF" title="Upload Foto Kegiatan" id="btnupload__<?php echo $id;?>">
			Upload Foto
		</div>
		...
		<div class="img-thumbnail-wrapper">
			...
			<div class="img-thumbnail" style="background-image: url(<?php echo base_url();?>public/files/spkp_aksi_dokumentasi/<?php echo $rs->id_kegiatan;?>/<?php echo $rs->filename;?>); background-size: cover; -webkit-transform: scale(1, 1) perspective(10000px) rotateX(0deg); opacity: 1; background-position: 50% 49%; background-repeat: no-repeat no-repeat;width:200px;height:150px">
				...
				<a href="<?php echo base_url();?>public/files/spkp_aksi_dokumentasi/<?php echo $rs->id_kegiatan;?>/<?php echo $rs->filename;?>" rel="group" class="fancybox" title="<?php echo $rs->judul;?>">
