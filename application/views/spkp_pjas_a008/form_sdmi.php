<script type="text/javascript">
    $(document).ready(function(){
	   ...
	   var source = {
            ...
            { name: 'kode_pbkpks', type: 'string' },
            { name: 'nama_sekolah', type: 'string' },
            { name: 'kepsek_nama', type: 'string' },
            { name: 'kepsek_nip', type: 'string' },
            { name: 'alamat', type: 'string' },
            { name: 'nilai', type: 'string' },
            { name: 'temuan', type: 'string' }
        ],
		url: "<?php echo base_url(); ?>spkp_pjas_a008/json_sdmi/{id}",
		cache: false,
		...
		{		
			...
			columns: [
				...
				// END
                { text: 'Sekolah', datafield: 'nama_sekolah', columntype: 'textbox', filtertype: 'textbox', width: '19%' },
                { text: 'Nama Kepsek', datafield: 'kepsek_nama', columntype: 'textbox', filtertype: 'textbox', width: '15%' },
                { text: 'NIP Kepsek', datafield: 'kepsek_nip', columntype: 'textbox', filtertype: 'list',width: '10%' },
                { text: 'Alamat', datafield: 'alamat', columntype: 'textbox', filtertype: 'textbox', width: '17%' },
                { text: 'Nilai', datafield: 'nilai', columntype: 'textbox', filtertype: 'textbox', width: '8%' },
                { text: 'Temuan', datafield: 'temuan', columntype: 'textbox', filtertype: 'textbox', width: '15%' },
                { text: 'Kode PBKP-KS', datafield: 'kode_pbkpks', columntype: 'textbox', filtertype: 'textbox', width: '7%' }
            ]
		});
	   ...
	   $('#btn_tambah_sdmi').click(function(){
		    ...
		    var offset = $("#jqxgrid_sdmi").offset();  
			...
			$.get("<?php echo base_url();?>spkp_pjas_a008/add_sdmi/{id}" , function(response) {
		...
		$('#printbutton_sdmi').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_pjas_a008/html_sdmi",
		...
		$('#excelbutton_sdmi').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_pjas_a008/excel_sdmi",
				success: function(response){
    ...
    function edit_sdmi(id){
        ...
        $("#popup_sdmi").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_pjas_a008/edit_sdmi/{id}/"+id , function(response) {
			$("#popup_content_sdmi").html("<div>"+response+"</div>");
		});
	}
    ...
    function save_edit_sdmi_dialog(content){
	   	$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_pjas_a008/doedit_sdmi/{id}",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog_sdmi();
    ...
    function save_add_sdmi_dialog(content){
        $.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_pjas_a008/doadd_sdmi/{id}",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog_sdmi();
    ...
    function dodel_sdmi(urut,id){
		if(confirm("Anda yakin akan menghapus data ("+urut+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/spkp_pjas_a008/dodel_sdmi/{id}/"+id,
				success: function(response){
					 if(response=="1"){
						 $('#clearfilteringbutton_sdmi').click();
                         $.notific8('Notification', {
...
<div id="popup_sdmi" style="display:none"><div id="popup_title_sdmi">Auditor</div><div id="popup_content_sdmi">{popup}</div></div>
<div>
