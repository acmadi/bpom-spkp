<script type="text/javascript">
    $(document).ready(function(){
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>spkp_pjas_a016/json_form",
		cache: false,
		...
		{		
			...
			columns: [
                ...
                { text: 'Bulan', datafield: 'bulan', columntype: 'textbox', filtertype: 'none', width: '8%', cellsrenderer: function(row){
                  var dataRecord = $("#jqxgrid").jqxGrid('getrowdata', row);
                  var arr_bln = ["","Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember"];
                  return "<div style='width:100%;padding:4px;text-align:left'>"+arr_bln[dataRecord.bulan]+"</div>";
                 }
                },
                { text: 'Balai', datafield: 'nama_balai', columntype: 'textbox', filtertype: 'list', width: '18%' },
                { text: 'Jabatan', datafield: 'penanggungjawab_jabatan', columntype: 'textbox', filtertype: 'textbox', width: '20%' },
                { text: 'Nama', datafield: 'penanggungjawab_nama', columntype: 'textbox', filtertype: 'textbox', width: '24%' },
                { text: 'NIP', datafield: 'penanggungjawab_nip', columntype: 'textbox', filtertype: 'textbox', width: '12%' }
            ]
		});
	   ...
	   $('#btn_tambah').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_pjas_a016/add_form" , function(response) {
		...
		$('#printbutton').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_pjas_a016/html_form",
		...
		$('#excelbutton').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_pjas_a016/excel_form",
				success: function(response){
    ...
    function edit_form(id){
		window.location.href = '<?php echo base_url(); ?>spkp_pjas_a016/edit/'+id;
	}

    function save_add_form_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_pjas_a016/doadd_form",
			data: content,
			success: function(response){
				 ...
				 if(data[0]=="1"){
                    ...
                    window.location.href = "<?php echo base_url(); ?>spkp_pjas_a016/edit/"+data[1];
				 }else{
					 $.notific8('Notification', {
					  life: 5000,
					  message: response,
    ...
    function dodel_form(urut,id){
		if(confirm("Anda yakin akan menghapus data ("+urut+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/spkp_pjas_a016/dodel_form/"+id,
				success: function(response){
					 if(response=="1"){
						 $('#clearfilteringbutton').click();
                         $.notific8('Notification', {
...
<div id="popup" style="display:none"><div id="popup_title">Form A016</div><div id="popup_content">{popup}</div></div>
<div>
