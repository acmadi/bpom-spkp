<script>
	$(function(){
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>spkp_personalia_kerjasama_nas/json_upload/{departemen}",
		cache: false,
		...
		{		
            ...
            rendertoolbar: function (toolbar) {
				...
				var departemen = $("<span style='float: left; margin-top: 5px; margin-right: 4px;'>Departmen: </span>");
				var departemeninput = $("<select class='jqx-input jqx-widget-content jqx-rc-all' id='filter_departemen' name='filter_departemen' style='height: 23px; float: left; width: 300px;' >{option_departemen}</select>");
				toolbar.append(container);
				container.append(departemen);
				container.append(departemeninput);
				departemeninput.change(function(){
					window.location.href="<?php echo base_url(); ?>spkp_personalia_kerjasama_nas/index/"+departemeninput.val();
                });
			},
            columns: [
			 ...
			 //   { text: 'Departmen', datafield: 'departemen', columntype: 'textbox', filtertype: 'textbox', width: '10%' },
                { text: 'Jenis', datafield: 'jenis', columntype: 'textbox', filtertype: 'textbox', width: '15%' },
	   ...
	   $('#btn_tambah_upload').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_personalia_kerjasama_nas/add_upload/{departemen}" , function(response) {
		...
		$('#printbutton_upload').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_personalia_kerjasama_nas/html_upload/{departemen}",
		...
		$('#excelbutton_upload').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_personalia_kerjasama_nas/excel_upload/{departemen}",
				success: function(response){
	...
	function edit_upload(id){
		...
		$("#popup_upload").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_personalia_kerjasama_nas/edit_upload/{departemen}/"+id , function(response) {
			$("#popup_content_upload").html("<div>"+response+"</div>");
		});
	}
	...
	function download(id){
		...
		$("#popup_upload").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_personalia_kerjasama_nas/download/"+id , function(response) {
			$("#popup_content_upload").html("<div>"+response+"</div>");
		});
	}
