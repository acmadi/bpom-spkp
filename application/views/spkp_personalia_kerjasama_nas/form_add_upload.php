    ...
    $(document).ready(function(){
		...
		$("[name='btn_simpan']").click(function(){
			...
			$.ajax({ 
				...
				contentType: false,
				processData: false,
				url: "<?php echo base_url()?>spkp_personalia_kerjasama_nas/do{action}_upload/{id}",
				data: data,
		...
		$("[name='btn_delete']").click(function(){
			if(confirm('Hapus dokumen ini?')){
				...
				$.ajax({ 
					...
					contentType: false,
					processData: false,
					url: "<?php echo base_url()?>spkp_personalia_kerjasama_nas/delete_upload/{id}",
					success: function(response){
						res = response.split("_");
						 if(res[0]=="OK"){
							 $.notific8('Notification', {
					...
					<tr>
						...
						<td>
                            <select size="1" name="departemen" style="margin: 0;width: 250px;height:25px;padding:2px">
                                <?php
                                $data_int = $this->spkp_personalia_kerjasama_nas_model->get_all_departmen_nas();
                                if($data_int->num_rows()>0){
                                    if(set_value('departemen')=="" && isset($departemen)){
                                        foreach($data_int->result() as $row_nas){
                                    ?>
                                        <option value="<?php echo $row_nas->id_departemen; ?>" <?php if($row_nas->id_departemen==$departemen) echo "Selected";  ?>><?php echo $row_nas->ket; ?></option>
                                    <?php
                                        }    
                                    }else{
                                        foreach($data_int->result() as $row_nas){
