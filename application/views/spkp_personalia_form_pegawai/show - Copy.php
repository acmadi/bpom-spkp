<script>
	$(function(){
       ...
       var source = {
            ...
            { name: 'bln_back',  type: 'number' },
            { name: 'bln_now',  type: 'number' },
            { name: 'thn_back',  type: 'number' },
            { name: 'thn_now',  type: 'number' }
        ],
        ...
        $("#jqxgrid").jqxGrid({		
			...
			columns: [
                ...
                { text: 'No', align: 'center', filtertype: 'none', width: '4%', cellsrenderer: function (row) {
				     var dataRecord = $("#jqxgrid").jqxGrid('getrowdata', row);
                     urut+=1;
                     return "<div style='width:100%;padding-top:4px;text-align:center'>"+urut+"</div>";
                 }
                },
                ...
                { text: 'Sisa Cuti <?php echo (date('Y')-1)?>', datafield: 'hari_back', filtertype: 'none', width: '12%', cellsalign: 'center', cellsrenderer: function(row){
                    ...
                    var bln_back = dataRecord.bln_back;
                    var thn_back = dataRecord.thn_back;
                    
                    var sum = hari_back+bln_back+thn_back;
                    
                    return "<div style='width:100%;padding-top:4px;text-align:center'>"+parseInt(12-sum)+"</div>";
                } },
                { text: 'Sisa Cuti <?php echo (date('Y'))?>', datafield: 'bln_back', filtertype: 'none', width: '12%', cellsalign: 'center', cellsrenderer: function(row){
                    ...
                    var bln_now = dataRecord.bln_now;
                    var thn_now = dataRecord.thn_now;
                    
                    var sum = hari_now+bln_now+thn_now;
                ...
                { text: 'Sisa Cuti Gabungan', datafield: 'thn_back', filtertype: 'none', width: '15%', cellsalign: 'center', cellsrenderer: function(row){
                    ...
                    var sum_back = hari_back+bln_back+thn_back;
                    
                    var hari_now = dataRecord.hari_now;
                    var bln_now = dataRecord.bln_now;
                    var thn_now = dataRecord.thn_now;
                    
                    var sum_now = hari_now+bln_now+thn_now;
                    
                    return "<div style='width:100%;padding-top:4px;text-align:center'>"+parseInt((12-sum_back)+(12-sum_now))+"</div>";
       ...
       var source = {
		...
		$('#cb_addcutibersama').click(function () {
			...
			$.get("<?php echo base_url();?>spkp_personalia_form_pegawai/add" , function(response) {
        ...
        $('#printbutton').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_personalia_form_pegawai/html",
        ...
        $('#excelbutton').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_personalia_form_pegawai/excel",
				success: function(response){
	...
	function cbedit(id){
		...
		$("#popup").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_personalia_form_pegawai/edit/"+id , function(response) {
			$("#popup_content").html("<div>"+response+"</div>");
		});
	}
	...
	function cbdel(id){
		if(confirm('Hapus dokumen ini?')){
			...
			$.ajax({ 
				...
				contentType: false,
				processData: false,
				url: "<?php echo base_url()?>spkp_personalia_form_pegawai/dodelete/"+id,
				success: function(response){
