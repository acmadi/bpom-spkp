    ...
    $(document).ready(function(){
		...
		$("[name='btn_simpan']").click(function(){
			...
			$.ajax({ 
				...
				contentType: false,
				processData: false,
				url: "<?php echo base_url()?>spkp_personalia_form_pegawai/do{action}_dokumen/{id}/{id_doc}",
				data: data,
		...
		$("[name='btn_delete']").click(function(){
			if(confirm('Hapus dokumen ini?')){
				...
				$.ajax({ 
					...
					contentType: false,
					processData: false,
					url: "<?php echo base_url()?>spkp_personalia_form_pegawai/dodelete_dokumen/{id}/{id_doc}",
					success: function(response){
						res = response.split("_");
						 if(res[0]=="OK"){
							 $.notific8('Notification', {
			...
			<td>
				...
				<table border="0" cellpadding="3" cellspacing="2">
					...
					<tr>
						<td>Keterangan</td>
						<td>:</td>
						<td><input type="text" size="80" name="ket" value="<?php 
