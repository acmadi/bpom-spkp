<script type="text/javascript">
    $(document).ready(function(){
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>spkp_pjas_a013/json_form",
		cache: false,
		...
		{		
			...
			columns: [
				...
				// END
                { text: 'Tahun', datafield: 'tahun', columntype: 'textbox', filtertype: 'textbox', width: '7%' },
                { text: 'Tanggal', datafield: 'ttd_tgl', columntype: 'date', filtertype: 'date', cellsformat: 'yyyy-MM-dd', width: '11%' },
                { text: 'Balai', datafield: 'nama_balai', columntype: 'textbox', filtertype: 'list', width: '30%' },
                { text: 'Nama', datafield: 'ttd_nama', columntype: 'textbox', filtertype: 'textbox', width: '28%' },
	   ...
	   $('#btn_tambah').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_pjas_a013/add_form" , function(response) {
    ...
    function edit_form(id){
		window.location.href = '<?php echo base_url(); ?>spkp_pjas_a013/edit/'+id;
	}

    function save_add_form_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_pjas_a013/doadd_form",
			data: content,
			success: function(response){
			     var data = response.split("_");
				 if(data[0]=="1"){
					 close_dialog();
					 window.location.href = "<?php echo base_url(); ?>spkp_pjas_a013/edit/"+data[1];
				 }else{
    ...
    function dodel_form(urut,id){
		if(confirm("Anda yakin akan menghapus data ("+urut+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/spkp_pjas_a013/dodel_form/"+id,
				success: function(response){
					 if(response=="1"){
...
<div id="popup" style="display:none"><div id="popup_title">Form A013</div><div id="popup_content">{popup}</div></div>
<div>
