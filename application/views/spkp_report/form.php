    ...
    $(document).ready(function(){
		...
		$("[name='btn_simpan']").click(function(){
			...
			$.ajax({ 
				...
				contentType: false,
				processData: false,
				url: "<?php echo base_url()?>spkp_report/do{action}_upload/{id}",
				data: data,
		...
		$("[name='btn_delete']").click(function(){
			if(confirm('Hapus dokumen ini?')){
				...
				$.ajax({ 
					...
					contentType: false,
					processData: false,
					url: "<?php echo base_url()?>spkp_report/delete_upload/{id}",
					success: function(response){
						res = response.split("_");
						 if(res[0]=="OK"){
							 $.notific8('Notification', {
					...
					<tr>
						...
						<td>
							<select name="tipe" style="height:25px;padding:2px;margin: 0;">
								<option value='LAKIP' <?php echo(isset($tipe) && $tipe=='LAKIP' ? "selected":"") ?>>LAKIP</option>
								<option value='Laporan Tahunan' <?php echo(isset($tipe) && $tipe=='Laporan Tahunan' ? "selected":"") ?>>Laporan Tahunan</option>							
							</select>
						</td>
					</tr>
                    <tr>
