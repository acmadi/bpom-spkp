<script type="text/javascript">
    $(document).ready(function(){
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>spkp_materi/json_materi",
		cache: false,
			...
			columns: [
				{ text: '#', align: 'center', filtertype: 'none', sortable: false, width: '5%', cellsrenderer: function (row) {
                     ...
                     if({add_permission}==true){
                        return "<div style='text-align: center;padding:3px'><a href='javascript:void(0);'><img border=0 src='<?php echo base_url(); ?>public/images/del.gif' onclick='dodel_materi("+dataRecord.urut+","+dataRecord.id+")'></a>  <a href='javascript:void(0);'><img border=0 src='<?php echo base_url(); ?>public/images/edt.gif' onclick='edit_materi("+dataRecord.id+");'></a></div>";
                     }else{
	   ...
	   $('#btn_tambah').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_materi/add_materi" , function(response) {
		...
		$('#printbutton').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_materi/html_materi",
		...
		$('#excelbutton').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_materi/excel_materi",
				success: function(response){
    ...
    function edit_materi(id){
        ...
        $("#popup").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_materi/edit_materi/"+id , function(response) {
			$("#popup_content").html("<div>"+response+"</div>");
		});
	}
	...
	function save_edit_materi_dialog(content){
	   	$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_materi/doedit_materi",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog();
    ...
    function save_add_materi_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_materi/doadd_materi",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog();
    ...
    function dodel_materi(urut,id){
		if(confirm("Anda yakin akan menghapus data ("+urut+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/spkp_materi/dodel_materi/"+id,
				success: function(response){
					 if(response=="1"){
						 $('#clearfilteringbutton').click();
                         $('#clearfilteringbutton_upload').click();
...
<div id="popup" style="display:none"><div id="popup_title">materi</div><div id="popup_content">{popup}</div></div>
<div>
