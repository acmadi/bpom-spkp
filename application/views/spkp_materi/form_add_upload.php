<script type="text/javascript">
    $(document).ready(function(){
		...
		$("[name='btn_simpan']").click(function(){
			...
			data.append('materi', $("[name='materi']").val());
			...
			$.ajax({ 
				...
				contentType: false,
				processData: false,
				url: "<?php echo base_url()?>spkp_materi/do{action}_upload/{id}",
				data: data,
		...
		$("[name='btn_delete']").click(function(){
			if(confirm('Hapus dokumen ini?')){
				...
				$.ajax({ 
					...
					contentType: false,
					processData: false,
					url: "<?php echo base_url()?>spkp_materi/delete_upload/{id}",
					success: function(response){
						res = response.split("_");
						 if(res[0]=="OK"){
							 $.notific8('Notification', {
			...
			<td>
                <table border="0" cellpadding="3" cellspacing="2" width='100%'>
					<tr>
						...
						<td>:</td>
						<td>
                            <select size="1" name="materi" style="margin: 0;width: 250px;height:25px;padding:2px">
                                <?php
                                $data_materi = $this->spkp_materi_model->get_all_materi();
                                if(set_value('materi')=="" && isset($materi)){
                                    foreach($data_materi as $row_materi){
                                ?>
                                    <option value="<?php echo $row_materi->id; ?>" <?php if($row_materi->id==$materi) echo "Selected";  ?>><?php echo $row_materi->nama; ?></option>
                                <?php
                                    }    
                                }else{
                                    foreach($data_materi as $row_materi){
