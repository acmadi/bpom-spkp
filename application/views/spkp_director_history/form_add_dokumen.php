<script type="text/javascript">
    $(document).ready(function(){
		...
		$("input[name='filename_file']").jqxInput({ theme: 'fresh', height: '22px', width: '200px' }); 
		...
		$("[name='btn_simpan']").click(function(){
			...
			jQuery.each($("[name='filename_file']")[0].files, function(i, file) {
			...
			$.ajax({ 
				...
				contentType: false,
				processData: false,
				url: "<?php echo base_url()?>spkp_director_history/do{action}_file/{id}",
				data: data,
		...
		$("[name='btn_delete']").click(function(){
			if(confirm('Hapus dokumen ini?')){
				...
				$.ajax({ 
					...
					contentType: false,
					processData: false,
					url: "<?php echo base_url()?>spkp_director_history/dodelete_file/{id}",
					success: function(response){
						res = response.split("_");
						 if(res[0]=="OK"){
							 $.notific8('Notification', {
				...
				<table border="0" cellpadding="3" cellspacing="2">
					<tr style="display: none;">
                        ...
                        <td>
                            ...
                            if($action=="add"){
                                ?>
                                <input type="text" size="5" maxlength="10" name="user" value="{id}" readonly=""/>
                                <?php
                            }else{
                                ?>
                                <input type="text" size="5" maxlength="10" name="user" value="{user}" readonly=""/>
                                <?php
                            }
                            ?>
                        </td>
					...
					<tr>
						<td>File</td>
						<td>:</td>
						<td><input type="file" size="10" name="filename_file" value="<?php 
								if(set_value('filename_file')=="" && isset($filename)){
								 	echo $filename;
								}else{
									echo  set_value('filename_file');
								}
								 ?>"/> *
						</td>
					</tr>
