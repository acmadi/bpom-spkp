<script>
	$(function(){
       ...
       $("#renstra_-_ditspkp").addClass("active");
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>spkp_renstra_dit/json_renstra_dit",
		cache: false,
	   ...
	   $('#btn_tambah').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_renstra_dit/add" , function(response) {
		...
		$('#printbutton').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_renstra_dit/html",
		...
		$('#excelbutton').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_renstra_dit/excel",
				success: function(response){
	...
	function edit(id){
		...
		$("#popup").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_renstra_dit/edit/"+id , function(response) {
			$("#popup_content").html("<div>"+response+"</div>");
		});
	}
	...
	function download(id){
		...
		$("#popup").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_renstra_dit/download/"+id , function(response) {
			$("#popup_content").html("<div>"+response+"</div>");
		});
	}
...
<div class="row-fluid">
   <div class="span12">
	   <h3 class="page-title">
		 Rencana Strategis DIT/SPKP
	   </h3>
   </div>
</div>
<div id="popup" style="display:none"><div id="popup_title">Rencana Strategis DIT/SPKP</div><div id="popup_content">{popup}</div></div>
<div>
