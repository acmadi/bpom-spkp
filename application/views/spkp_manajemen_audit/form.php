    ...
    $(document).ready(function(){
		...
		$("[name='btn_simpan']").click(function(){
			...
			$.ajax({ 
				...
				contentType: false,
				processData: false,
				url: "<?php echo base_url()?>spkp_manajemen_audit/do{action}_upload/{id}",
				data: data,
		...
		$("[name='btn_delete']").click(function(){
			if(confirm('Hapus dokumen ini?')){
				...
				$.ajax({ 
					...
					contentType: false,
					processData: false,
					url: "<?php echo base_url()?>spkp_manajemen_audit/delete_upload/{id}",
					success: function(response){
						res = response.split("_");
						 if(res[0]=="OK"){
							 $.notific8('Notification', {
			...
			<td>
                <table border="0" cellpadding="3" cellspacing="2" width='100%'>
					...
					<tr>
						...
						<td>:</td>
						<td>
							<select name="tipe" style="height:25px;padding:2px;margin: 0;">
								<option value='internal' <?php echo(isset($tipe) && $tipe=='internal' ? "selected":"") ?>>Internal</option>
								<option value='external' <?php echo(isset($tipe) && $tipe=='external' ? "selected":"") ?>>External</option>							
							</select>
						</td>
					</tr>
                    <tr>
