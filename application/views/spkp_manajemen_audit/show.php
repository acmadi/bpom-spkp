<script>
	$(function(){
       ...
       $("#qms_audit_mutu").addClass("active");
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>spkp_manajemen_audit/json_judul/{thn}",
		cache: false,
		...
		{		
            ...
            rendertoolbar: function (toolbar) {
				...
				thninput.change(function(){
					window.location.href="<?php echo base_url(); ?>spkp_manajemen_audit/index/"+thninput.val();
                });
			},
			columns: [
				...
				{ text: 'Tipe', datafield: 'tipe', columntype: 'dropdownlist', filtertype: 'list', width: '12%' },
                { text: 'Keterangan', datafield: 'ket_file', columntype: 'textbox', filtertype: 'textbox', width: '19%' },
				{ text: 'File', datafield: 'filename', columntype: 'textbox', filtertype: 'textbox', width: '12%' },
	   ...
	   $('#btn_tambah_upload').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_manajemen_audit/add_upload" , function(response) {
		...
		$('#printbutton_upload').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_manajemen_audit/html_upload/{thn}",
		...
		$('#excelbutton_upload').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_manajemen_audit/excel_upload/{thn}",
				success: function(response){
	...
	function edit_upload(id){
		...
		$("#popup_upload").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_manajemen_audit/edit_upload/"+id , function(response) {
			$("#popup_content_upload").html("<div>"+response+"</div>");
		});
	}
	...
	function download(id){
		...
		$("#popup_upload").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_manajemen_audit/download/"+id , function(response) {
			$("#popup_content_upload").html("<div>"+response+"</div>");
		});
	}
