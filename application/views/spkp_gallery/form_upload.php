<script type="text/javascript">
   $(document).ready(function(){
		...
		$("[name='btn_simpan']").click(function(){
			...
			data.append('cover', $("[name='cover']").val());
			...
			$.ajax({ 
				...
				contentType: false,
				processData: false,
				url: "<?php echo base_url()?>spkp_gallery/do{action}/{id}",
				data: data,
				success: function(response){
					var res=response.split("_");
					 if(res[0]=="OK"){
						...
						$("#uploadloader").hide();
						$("#uploaddiv").show("fade");
						window.location.href = "<?php echo base_url();?>spkp_gallery/show/{id}";
					 }else{
						 $.notific8('Notification', {
						  life: 5000,
		...
		$("[name='btn_delete']").click(function(){
			if(confirm('Hapus dokumen ini?')){
				...
				$.ajax({ 
					...
					contentType: false,
					processData: false,
					url: "<?php echo base_url()?>spkp_gallery/dodelete/{id}",
					success: function(response){
			...
			<td>
				...
				<table border="0" cellpadding="3" cellspacing="2">
					...
					<tr>
						...
						<td>:</td>
						<td>
							<select name="cover" style="height:25px;padding:2px;">
								<option value='1' <?php echo(isset($cover) && $status==1 ? "selected":"") ?>>Yes</option>
								<option value='0' <?php echo(isset($cover) && $status==0 ? "selected":"") ?>>No</option>
							</select>
						</td>
					</tr>
				</table>
