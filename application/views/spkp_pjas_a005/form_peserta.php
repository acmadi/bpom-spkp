<script type="text/javascript">
    $(document).ready(function(){
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>spkp_pjas_a005/json_peserta/{id}",
		cache: false,
		...
		{		
			...
			columns: [
                ...
                { text: 'Jabatan', datafield: 'jabatan', columntype: 'textbox', filtertype: 'textbox', width: '46%' }
            ]
		});
	   ...
	   $('#btn_tambah_peserta').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_pjas_a005/add_peserta/{id}" , function(response) {
				$("#popup_content_peserta").html("<div>"+response+"</div>");
			});
		});

		$('#printbutton_peserta').click(function () {
  			$("#popup_content_peserta").html("<div style='text-align:center'><br><br><br><br><img src='<?php echo base_url();?>media/images/indicator.gif' alt='loading content.. '><br>loading</div>");
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_pjas_a005/html_peserta",
				success: function(response){
					$("#popup_content_peserta").html(response);
				}
			 }); 		
		...
		$('#excelbutton_peserta').click(function () {
			//Collecting filter information into querystring
			var datastring="pagenum=0&pagesize=9999&groupscount=0&groupscount=10&recordstartindex=0&";
			var filter = $("#jqxgrid_peserta").jqxGrid('getfilterinformation');
			k=0;
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_pjas_a005/excel_peserta",
				success: function(response){
    ...
    function edit_peserta(id){
        ...
        $("#popup_peserta").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_pjas_a005/edit_peserta/{id}/"+id , function(response) {
			$("#popup_content_peserta").html("<div>"+response+"</div>");
		});
	}
    ...
    function save_edit_peserta_dialog(content){
	   	$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_pjas_a005/doedit_peserta/{id}",
			data: content,
			success: function(response){
				 if(response=="1"){
    ...
    function save_add_peserta_dialog(content){
        $.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_pjas_a005/doadd_peserta/{id}",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog_peserta();
    ...
    function dodel_peserta(urut,id){
		if(confirm("Anda yakin akan menghapus data ("+urut+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/spkp_pjas_a005/dodel_peserta/{id}/"+id,
				success: function(response){
					 if(response=="1"){
						 $('#clearfilteringbutton_peserta').click();
                         $.notific8('Notification', {
...
<div id="popup_peserta" style="display:none"><div id="popup_title_peserta">Peserta Bimtek</div><div id="popup_content_peserta">{popup}</div></div>
<div>
