<script type="text/javascript">
    $(document).ready(function(){
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>spkp_pjas_a005/json_form",
		cache: false,
		...
		{		
			...
			columns: [
				...
				// END
                { text: 'Balai', datafield: 'nama_balai', columntype: 'textbox', filtertype: 'list', width: '25%' },
                { text: 'Tempat', datafield: 'tempat', columntype: 'textbox', filtertype: 'textbox', width: '20%' },
                { text: 'Tanggal', datafield: 'tanggal', columntype: 'textbox', filtertype: 'textbox', width: '16%' },
	   ...
	   $('#btn_tambah').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_pjas_a005/add_form" , function(response) {
		...
		$('#printbutton').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_pjas_a005/html_form",
		...
		$('#excelbutton').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_pjas_a005/excel_form",
				success: function(response){
    ...
    function edit_form(id){
		window.location.href = '<?php echo base_url(); ?>spkp_pjas_a005/edit/'+id;
	}

    function save_add_form_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_pjas_a005/doadd_form",
			data: content,
			success: function(response){
				 ...
				 if(data[0]=="1"){
                    ...
                    window.location.href = "<?php echo base_url(); ?>spkp_pjas_a005/edit/"+data[1];
				 }else{
					 $.notific8('Notification', {
					  life: 5000,
					  message: response,
    ...
    function dodel_form(urut,id){
		if(confirm("Anda yakin akan menghapus data ("+urut+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/spkp_pjas_a005/dodel_form/"+id,
				success: function(response){
					 if(response=="1"){
						 $('#clearfilteringbutton').click();
...
<div id="popup" style="display:none"><div id="popup_title">Form A005</div><div id="popup_content">{popup}</div></div>
<div>
