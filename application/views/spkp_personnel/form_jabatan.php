<script type="text/javascript">
    $(document).ready(function(){
       $('#clearfilteringbutton, #refreshdatabutton, #printbutton, #excelbutton, #btnBack, #btnTambah').jqxButton({ height: 25, theme: theme });
       ...
       var source = {
		...
		url: "<?php echo base_url(); ?>index.php/spkp_personnel/json_jabatan/{id}",
		cache: false,
        ...
        $('#btnTambah').click(function(){
			...
			$.get("<?php echo base_url();?>index.php/spkp_personnel/add_jabatan" , function(response) {
        ...
        $('#printbutton').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_personnel/html_jabatan/{id}",
        ...
        $('#excelbutton').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_personnel/excel_jabatan/{id}",
				success: function(response){
    ...
    function edit_jabatan(id,id_jabatan){
        ...
        $("#popup").jqxWindow('open');
		$.get("<?php echo base_url();?>index.php/spkp_personnel/edit_jabatan/"+id+"/"+id_jabatan , function(response) {
			$("#popup_content").html("<div>"+response+"</div>");
		});
	}
	...
	function save_edit_jabatan_dialog(content){
	   	$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_personnel/doedit_jabatan",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog();
    ...
    function save_add_jabatan_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_personnel/doadd_jabatan/{id}",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog();
    ...
    function dodel_jabatan(urut,id,id_jabatan){
		if(confirm("Anda yakin akan menghapus data ("+urut+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/spkp_personnel/dodel_jabatan/"+id+"/"+id_jabatan,
				success: function(response){
					 if(response=="1"){
						 $('#clearfilteringbutton').click();
						 $.notific8('Notification', {
...
<div style="padding:8px">
	<div style="width:98%;background-color:#DDDDDD;-moz-border-radius:5px;border-radius:5px;padding:2px;border:3px solid #ebebeb;">
	    <input style="padding: 5px;" value=" Back" id="btnBack" type="button" />
