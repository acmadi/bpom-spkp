<script type="text/javascript">
    $(document).ready(function(){
	   ...
	   var source = {
            ...
            { name: 'komunitas', type: 'string' },
            { name: 'jenis', type: 'string' },
            { name: 'kie_peserta', type: 'string' },
            { name: 'kie_materi', type: 'string' },
            { name: 'dokumentasi', type: 'string' },
            { name: 'evaluasi', type: 'string' }
        ],
		url: "<?php echo base_url(); ?>spkp_pjas_a019/json_sdmi/{id}",
		cache: false,
		...
		{		
			...
			columns: [
                ...
                { text: 'Nama', datafield: 'nama', columntype: 'textbox', filtertype: 'textbox', width: '15%' },
                { text: 'NPSN', datafield: 'npsn', columntype: 'textbox', filtertype: 'textbox',width: '12%' },
                { text: 'Alamat', datafield: 'alamat', columntype: 'textbox', filtertype: 'textbox', width: '16%' },
                { text: 'Jenis Informasi', datafield: 'jenis', columntype: 'textbox', filtertype: 'textbox', width: '14%' },
                { text: 'Peserta KIE', datafield: 'kie_peserta', columntype: 'textbox', filtertype: 'textbox', width: '12%' },
                { text: 'Materi KIE', datafield: 'kie_materi', columntype: 'textbox', filtertype: 'textbox', width: '13%' }
            ]
		});
	   ...
	   $('#btn_tambah_sdmi').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_pjas_a019/add_sdmi/{id}" , function(response) {
		...
		$('#printbutton_sdmi').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_pjas_a019/html_sdmi",
		...
		$('#excelbutton_sdmi').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_pjas_a019/excel_sdmi",
				success: function(response){
    ...
    function edit_sdmi(id){
        ...
        $("#popup_sdmi").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_pjas_a019/edit_sdmi/{id}/"+id , function(response) {
			$("#popup_content_sdmi").html("<div>"+response+"</div>");
		});
	}
    ...
    function save_edit_sdmi_dialog(content){
	   	$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_pjas_a019/doedit_sdmi/{id}",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog_sdmi();
    ...
    function save_add_sdmi_dialog(content){
        $.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_pjas_a019/doadd_sdmi/{id}",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog_sdmi();
    ...
    function dodel_sdmi(urut,id){
		if(confirm("Anda yakin akan menghapus data ("+urut+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/spkp_pjas_a019/dodel_sdmi/{id}/"+id,
				success: function(response){
					 if(response=="1"){
						 $('#clearfilteringbutton_sdmi').click();
                         $.notific8('Notification', {
