...
<tr>
	<td>
		<button type="button" class=btn onclick="document.location.href='<?php echo base_url()?>index.php/admin_content/editnews_form/file_id/{file_id}'">Tambah</button>
	...
	<?php if($page_count>1) { ?>|| Pindah halaman : <select class=input onchange="document.location.href='<?php echo base_url()?>index.php/admin_content/editnews/{segments}page/'+this.value+'.html';">';
	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%" class="tbl">
		...
		<?php foreach($query as $row):?>
			<tr onMouseOver="bgColor='#EEEEEE'" onmouseout="bgColor='#FFFFFF'">
				...
				<td class=tbl_list align=center><a href="<?php echo base_url()?>index.php/admin_content/editnews_form/file_id/{file_id}/id/<?php echo $row->id?>" title="Ubah"><img src="<?php echo base_url()?>media/images/16_edit.gif" /></a></td>
				<td class="tbl_list" align="center"><a href="#" onclick="if(confirm('Hapus data ini?'))document.location.href='<?php echo base_url()?>index.php/admin_content/dodelnews/{file_id}/<?php echo $row->id?>'" title="Hapus"><img src="<?php echo base_url()?>media/images/16_del.gif" /></a></td>
			</tr>
		<?php endforeach;?>
		</tbody>
	</table>
