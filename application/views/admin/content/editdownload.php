	...
	$(function() {
		...
		$(function(){
			...
			new AjaxUpload($('#linkimages'), {
				action: '<?php echo base_url()?>index.php/admin_content/douploadifile/{file_id}/download',
				name: 'uploadfile',
				onSubmit: function(file, ext){
					$('#linkimages_alert').show('fold',500);
					$('#linkimages_alert').html('Uploading File...');
				},
				onComplete: function(file, response){
					stat = response.substr(0,7)
					filename = response.substr(10)
					if(stat==="success"){
						$('#links').val("<?php echo base_url()?>media/files/download/{file_id}/"+filename);
						$('#linkimages_alert').html('<div align=right onClick="$(\'#linkimages_alert\').hide(\'fold\',500);" style="color:red;font-weight:bold">X</div><br>Upload File OK');
					} else{
...
<div class="clear">&nbsp;</div>
<form action="<?php echo base_url()?>index.php/admin_content/doeditdownload/file_id/{file_id}/id/{id}" method="POST" name="frmFiles" enctype="multipart/form-data">
	<button type="submit" class=btn>Simpan</button>
	<button type="reset" class=btn>Ulang</button>
	<button type="button" class=btn onclick="document.location.href='<?php echo base_url()?>index.php/admin_content/editdownload/file_id/{file_id}';">Kembali</button>
	<br />
	<br />
		...
		<tr>
			<td>
				...
				<table border="0" cellpadding="3" cellspacing="2">
					...
					?>
						<tr>
							<td>Description <b><?php echo $row['lang']?></b></td>
							<td>:</td>
					...
					<tr>
						<td><input class=input type="text" size="100" name="links" id="links" value="<?php echo $links?>" readonly/></td>
					</tr>
					<tr>
