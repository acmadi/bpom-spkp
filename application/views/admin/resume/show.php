<script>
	$(function(){
		...
		$('#flexmaster').flexigrid({
			url: 'admin_master_resume/json',
			dataType: 'json',
			colModel : [
				{display: 'Proses', name : 'PROSES', width : 50, sortable : false, align: 'center'},
				{display: 'ID Resume', name : 'id_resume', width : 65, sortable : true, align: 'left'},
				{display: 'Desc Resume', name : 'desc_resume', width : 300, sortable : true, align: 'left'}
			],
			searchitems : [
				{display: 'ID Resume', name : 'id_resume', isdefault: true},
				{display: 'Desc Resume', name : 'desc_resume'},
			],
			usepager: true,
			rp: 15,
			rpOptions: [10,15],
		...
		$('#btn_tambah',sDiv).click(function(){
			...
			$.get("<?php echo base_url();?>index.php/admin_master_resume/add" , function(response) {
	...
	function save_add_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/admin_master_resume/doadd",
			data: content,
	...
	function edit(id){
		...
		$.get("<?php echo base_url();?>index.php/admin_master_resume/edit/"+id , function(response) {
			$("#popup").html("<div>"+response+"</div>");
		});
	}
	...
	function save_edit_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/admin_master_resume/doedit",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog();
	...
	function dodel(id){
		if(confirm("Anda yakin akan menghapus data ("+id+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/admin_master_resume/dodel/"+id,
				success: function(response){
					 if(response=="1"){
						 var sDiv = $('.sDiv');
						 $('input[value=Clear]',sDiv).click();
