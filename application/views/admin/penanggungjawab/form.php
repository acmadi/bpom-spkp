<script>
	$(document).ready(function(){
		$("#uid").change(function(){
			var str = $("#uid option:selected").text().split(" - ");

			$("input[name='nama']").val(str[1]);
			$("input[name='nip']").val(str[2]);
		});
	});
			...
			<td>
				...
				<table border="0" cellpadding="3" cellspacing="2">
					...
					<tr>
						...
						<td>:</td>
						<td>
                            <select size="1" id="uid" name="uid" class="input">
                                <?php
                                    $data=$this->admin_master_penanggungjawab_model->get_user();
                                    foreach($data as $row){
                                        if(set_value('uid')=="" && isset($uid)){
                                            ?>
                                                <option value="<?php echo $row->id; ?>" <?php if($row->id==$uid) echo "selected"; ?>><?php echo $row->username; ?> - <?php echo $row->name_display; ?> - <?php echo $row->nip; ?></option>
                                            <?php
                                        }else{
                                            ?>
                                                <option value="<?php echo $row->id; ?>"><?php echo $row->username; ?> - <?php echo $row->name_display; ?> - <?php echo $row->nip; ?></option>
                                            <?php
                                        }
                                    }   
                                ?>
					...
					<tr>
						<td>NIP</td>
						<td>:</td>
						<td><input class=input type="text" size="30" name="nip" name="nip" value="<?php 
