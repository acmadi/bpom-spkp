<script>
	$(function(){
		...
		$('#flexmaster').flexigrid({
			url: 'admin_master_tujuan/json',
			dataType: 'json',
			colModel : [
				{display: 'Proses', name : 'PROSES', width : 50, sortable : false, align: 'center'},
				{display: 'ID Tujuan', name : 'a.id_tujuan', width : 65, sortable : true, align: 'left'},
				{display: 'Desc Tujuan', name : 'a.desc_tujuan', width : 670, sortable : true, align: 'left'},
                {display: 'Group', name : 'b.name', width : 100, sortable : true, align: 'left'}
			],
			searchitems : [
				{display: 'ID Tujuan', name : 'a.id_tujuan', isdefault: true},
				{display: 'Desc Tujuan', name : 'a.desc_tujuan'},
                {display: 'Group', name : 'b.name'},
		...
		$('#btn_tambah',sDiv).click(function(){
			...
			$.get("<?php echo base_url();?>index.php/admin_master_tujuan/add" , function(response) {
	...
	function save_add_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/admin_master_tujuan/doadd",
			data: content,
	...
	function edit(id){
		...
		$.get("<?php echo base_url();?>index.php/admin_master_tujuan/edit/"+id , function(response) {
			$("#popup").html("<div>"+response+"</div>");
		});
	}
	...
	function save_edit_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/admin_master_tujuan/doedit",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog();
	...
	function dodel(id){
		if(confirm("Anda yakin akan menghapus data ("+id+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/admin_master_tujuan/dodel/"+id,
				success: function(response){
					 if(response=="1"){
						 var sDiv = $('.sDiv');
						 $('input[value=Clear]',sDiv).click();
