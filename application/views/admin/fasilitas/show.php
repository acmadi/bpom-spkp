<script>
	$(function(){
		...
		$('#flexmaster').flexigrid({
			url: 'admin_master_fasilitas/json',
			dataType: 'json',
			colModel : [
				{display: 'Proses', name : 'PROSES', width : 50, sortable : false, align: 'center'},
				{display: 'ID Fasilitas', name : 'id_fasilitas', width : 90, sortable : true, align: 'left'},
				{display: 'Nama Fasilitas', name : 'nama_fasilitas', width : 230, sortable : true, align: 'left'}
			],
			searchitems : [
				{display: 'ID Fasilitas', name : 'id_fasilitas', isdefault: true},
				{display: 'Nama Fasilitas', name : 'nama_fasilitas'},
			],
			usepager: true,
			rp: 15,
			rpOptions: [10,15],
		...
		$('#btn_tambah',sDiv).click(function(){
			...
			$.get("<?php echo base_url();?>index.php/admin_master_fasilitas/add" , function(response) {
	...
	function save_add_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/admin_master_fasilitas/doadd",
			data: content,
	...
	function edit(id){
		...
		$.get("<?php echo base_url();?>index.php/admin_master_fasilitas/edit/"+id , function(response) {
			$("#popup").html("<div>"+response+"</div>");
		});
	}
	...
	function save_edit_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/admin_master_fasilitas/doedit",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog();
	...
	function dodel(id){
		if(confirm("Anda yakin akan menghapus data ("+id+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/admin_master_fasilitas/dodel/"+id,
				success: function(response){
					 if(response=="1"){
						 var sDiv = $('.sDiv');
						 $('input[value=Clear]',sDiv).click();
