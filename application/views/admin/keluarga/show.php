<script>
	$(function(){
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>index.php/admin_master_keluarga/json_keluarga",
		cache: false,
        ...
        $("#jqxgrid").jqxGrid({		
			...
			columns: [
				...
				{ text: 'ID ', datafield: 'id', filtertype: 'textbox',  width: 50},
        ...
        $('#btn_tambah').click(function(){
			...
			$.get("<?php echo base_url();?>index.php/admin_master_keluarga/add" , function(response) {
	...
	function save_add_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/admin_master_keluarga/doadd",
			data: content,
	...
	function edit(id){
        ...
        $("#popup").jqxWindow('open');
		$.get("<?php echo base_url();?>index.php/admin_master_keluarga/edit/"+id , function(response) {
			$("#popup_content").html("<div>"+response+"</div>");
		});
	}
	...
	function save_edit_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/admin_master_keluarga/doedit",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog();
	...
	function dodel(id){
		if(confirm("Anda yakin akan menghapus data ("+id+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/admin_master_keluarga/dodel/"+id,
				success: function(response){
					 if(response=="1"){
						 $('#clearfilteringbutton').click();
						 $.notific8('Notification', {
