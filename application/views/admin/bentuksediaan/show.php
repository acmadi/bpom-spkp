<script>
	$(function(){
		...
		$('#flexmaster').flexigrid({
			url: 'admin_master_bentuksediaan/json',
			dataType: 'json',
			colModel : [
				{display: 'Proses', name : 'PROSES', width : 50, sortable : false, align: 'center'},
				{display: 'ID Bentuk Sediaan', name : 'id_sediaan', width : 105, sortable : true, align: 'left'},
				{display: 'Nama Bentuk Sediaan', name : 'nama_sediaan', width : 400, sortable : true, align: 'left'},
				{display: 'Jenis', name : 'id_jenis', width : 200, sortable : true, align: 'left'}
			],
			searchitems : [
				{display: 'ID Bentuk Sediaan', name : 'id_sediaan', isdefault: true},
				{display: 'Nama Bentuk Sediaan', name : 'nama_sediaan'},
				{display: 'Jenis', name : 'id_jenis'}
			],
			usepager: true,
			rp: 15,
		...
		$('#btn_tambah',sDiv).click(function(){
			...
			$.get("<?php echo base_url();?>index.php/admin_master_bentuksediaan/add" , function(response) {
	...
	function save_add_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/admin_master_bentuksediaan/doadd",
			data: content,
	...
	function edit(id){
		...
		$.get("<?php echo base_url();?>index.php/admin_master_bentuksediaan/edit/"+id , function(response) {
			$("#popup").html("<div>"+response+"</div>");
		});
	}
	...
	function save_edit_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/admin_master_bentuksediaan/doedit",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog();
	...
	function dodel(id){
		if(confirm("Anda yakin akan menghapus data ("+id+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/admin_master_bentuksediaan/dodel/"+id,
				success: function(response){
					 if(response=="1"){
						 var sDiv = $('.sDiv');
						 $('input[value=Clear]',sDiv).click();
