<script>
	$(function(){
		...
		$('#flexmaster').flexigrid({
			url: 'admin_master_statusindustri/json',
			dataType: 'json',
			colModel : [
				{display: 'Proses', name : 'PROSES', width : 50, sortable : false, align: 'center'},
				{display: 'ID Status', name : 'id_status', width : 55, sortable : true, align: 'left'},
				{display: 'Nama', name : 'nama', width : 230, sortable : true, align: 'left'}
			],
			searchitems : [
				{display: 'ID Status', name : 'id_status', isdefault: true},
		...
		$('#btn_tambah',sDiv).click(function(){
			...
			$.get("<?php echo base_url();?>index.php/admin_master_statusindustri/add" , function(response) {
	...
	function save_add_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/admin_master_statusindustri/doadd",
			data: content,
	...
	function edit(id){
		...
		$.get("<?php echo base_url();?>index.php/admin_master_statusindustri/edit/"+id , function(response) {
			$("#popup").html("<div>"+response+"</div>");
		});
	}
	...
	function save_edit_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/admin_master_statusindustri/doedit",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog();
	...
	function dodel(id){
		if(confirm("Anda yakin akan menghapus data ("+id+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/admin_master_statusindustri/dodel/"+id,
				success: function(response){
					 if(response=="1"){
						 var sDiv = $('.sDiv');
						 $('input[value=Clear]',sDiv).click();
