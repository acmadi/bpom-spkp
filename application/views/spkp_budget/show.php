<script>
	$(function(){
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>spkp_budget/json_budget",
		cache: false,
		...
		{		
			...
			columns: [
			    ...
			    { text: 'Jenis', datafield: 'nama', columntype: 'textbox', filtertype: 'list', width: '15%' },
                { text: 'Tahun', datafield: 'tahun', columntype: 'textbox', filtertype: 'list', width: '6%', align: 'center', cellsalign: 'center' },
	   ...
	   $('#btn_tambah').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_budget/add" , function(response) {
		...
		$('#printbutton').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_budget/html",
		...
		$('#excelbutton').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_budget/excel",
				success: function(response){
	...
	function edit(id){
		...
		$("#popup").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_budget/edit/"+id , function(response) {
			$("#popup_content").html("<div>"+response+"</div>");
		});
	}
	...
	function download(id){
		...
		$("#popup").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_budget/download/"+id , function(response) {
			$("#popup_content").html("<div>"+response+"</div>");
		});
	}
...
<div id="popup" style="display:none"><div id="popup_title">Anggaran</div><div id="popup_content">{popup}</div></div>
<div>
