    ...
    $(document).ready(function(){
		...
		$("[name='btn_simpan']").click(function(){
			...
			$.ajax({ 
				...
				contentType: false,
				processData: false,
				url: "<?php echo base_url()?>spkp_budget/do{action}/{id}",
				data: data,
		...
		$("[name='btn_delete']").click(function(){
			if(confirm('Hapus dokumen ini?')){
				...
				$.ajax({ 
					...
					contentType: false,
					processData: false,
					url: "<?php echo base_url()?>spkp_budget/delete/{id}",
					success: function(response){
						res = response.split("_");
						 if(res[0]=="OK"){
							 $.notific8('Notification', {
			...
			<td>
                <table border="0" cellpadding="3" cellspacing="2">
                    ...
                    <tr>
                        ...
                        <td>:</td>
                        <td>
                            <select size="1" name="jenis" style="width: 200px;height:25px;padding:2px;margin: 0;">
                                <?php
                                $data_jenis = $this->spkp_budget_model->get_all_jenis();
                                if(set_value('jenis')=="" && isset($jenis)){
                                    foreach($data_jenis as $row_jenis){
                                    ?>
                                    <option value="<?php echo $row_jenis->id_jenis; ?>" <?php if($row_jenis->id_jenis==$jenis) echo "Selected";  ?>><?php echo $row_jenis->nama; ?></option>
                                    <?php
                                    }
                                }else{
                                    foreach($data_jenis as $row_jenis){
                                    ?>
                                    <option value="<?php echo $row_jenis->id_jenis; ?>"><?php echo $row_jenis->nama; ?></option>
                                    <?php
                                    }
                                }
                                ?>
