<script>
	$(function(){
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>spkp_pjas_petunjukteknis/json_upload/{thn}",
		cache: false,
		...
		{		
            ...
            rendertoolbar: function (toolbar) {
				...
				thninput.change(function(){
					window.location.href="<?php echo base_url(); ?>spkp_pjas_petunjukteknis/index/2/"+thninput.val();
                });
			},
			columns: [
	   ...
	   $('#btn_tambah_upload').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_pjas_petunjukteknis/add_upload/{thn}" , function(response) {
	...
	function edit_upload(id){
		...
		$("#popup_upload").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_pjas_petunjukteknis/edit_upload/{thn}/"+id , function(response) {
			$("#popup_content_upload").html("<div>"+response+"</div>");
		});
	}
