<script type="text/javascript">
    ...
    function save_profile(){
        ...
        $.ajax({
		   ...
		   contentType: false,
		   processData: false,
           url: "<?php echo base_url(); ?>spkp_director_history/doedit_profile/{id}",
           data: data,
           success: function(response){
                if(response=="1"){
                   ...
                   $.ajax({
                     type: "POST",
                     url: "<?php echo base_url(); ?>spkp_director_history/load_form_profile/{id}",
                     success: function(response){
                        $("#divFormProfile").show();
...
<div id="divFormProfile" style="padding:8px">
        ...
        <div style="position:relative;float:right;right:200px;border:15px solid #EFEFEF;top:50px">
    		...
    		if($image==""){
    			...
    			?>
    			<img src="<?php echo base_url(); ?>media/images/director/{image}" width="200" height="200" style="border: 1px solid #404040;"/>
    			<?php
    		}
    		?>
	    </div>
        ...
        <tr>
          ...
          <td>
               <?php
               if($data_agama = $this->spkp_director_history_model->get_agama($agama)){
         ...
         <tr>
           ...
           <td>
                <?php
                if($data_kawin = $this->spkp_director_history_model->get_kawin($kawin)){
		 ...
		 <td>
			<table border='0' width='100%' cellpadding='7' cellspacing='2' style="font-size: 12px;color: black">
				  ...
				  <tr>
					  ...
					  <td>
						 <?php
                         if($data_propinsi = $this->spkp_director_history_model->get_propinsi($propinsi)){
				   ...
				   <tr>
					   ...
					   <td>
						  <?php
                          if($data_kota = $this->spkp_director_history_model->get_kota($kota)){
				   ...
				   <tr>
					   ...
					   <td>
						  <?php
                          if($data_kec = $this->spkp_director_history_model->get_kecamatan($kecamatan)){
				   ...
				   <tr>
						...
						<td>
						   <?php
                           if($data_desa = $this->spkp_director_history_model->get_desa($desa)){
