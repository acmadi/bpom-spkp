<script type="text/javascript">
    $(document).ready(function(){
		...
		$("input[name='tmpt']").jqxInput({ theme: 'fresh', height: '22px', width: '125px' });
    ...
    function save_add_rekap_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_pjas_a023/doadd",
			data: content,
			success: function(response){
			var data = response.split("_");
				 if(data[0]=="1"){
					window.location.href = "<?php echo base_url(); ?>spkp_pjas_a023/edit/"+data[1];
				 }else{
