<script type="text/javascript">
    $(document).ready(function(){
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>spkp_pjas_a020/json_form",
		cache: false,
		...
		{		
			...
			columns: [
				...
				// END
                { text: 'Tanggal', datafield: 'tgl', columntype: 'date', filtertype: 'date', cellsformat: 'yyyy-MM-dd', width: '11%' },
                { text: 'Penyelenggara', datafield: 'penyelenggara', columntype: 'textbox', filtertype: 'textbox', width: '25%' },
                { text: 'Tempat', datafield: 'tempat', columntype: 'textbox', filtertype: 'textbox', width: '20%' },
                { text: 'Nama', datafield: 'ttd_nama', columntype: 'textbox', filtertype: 'textbox', width: '20%' },
                { text: 'NIP', datafield: 'ttd_nip', columntype: 'textbox', filtertype: 'textbox', width: '15%' }
            ]
		});
	   ...
	   $('#btn_tambah').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_pjas_a020/add_form" , function(response) {
    ...
    function edit_form(id){
		window.location.href = '<?php echo base_url(); ?>spkp_pjas_a020/edit/'+id;
	}

    function save_add_form_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_pjas_a020/doadd_form",
			data: content,
			success: function(response){
			     var data = response.split("_");
				 if(data[0]=="1"){
					 close_dialog();
					 window.location.href = "<?php echo base_url(); ?>spkp_pjas_a020/edit/"+data[1];
				 }else{
    ...
    function dodel_form(urut,id){
		if(confirm("Anda yakin akan menghapus data ("+urut+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/spkp_pjas_a020/dodel_form/"+id,
				success: function(response){
					 if(response=="1"){
...
<div id="popup" style="display:none"><div id="popup_title">Form A020</div><div id="popup_content">{popup}</div></div>
<div>
