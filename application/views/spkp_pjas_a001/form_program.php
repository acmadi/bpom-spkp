<script type="text/javascript">
    $(document).ready(function(){
	   ...
	   var source = {
			...
			datafields: [
			{ name: 'urut'},
            { name: 'id_program', type: 'number' },
			{ name: 'strategi', type: 'number' },
            { name: 'nama', type: 'string' },
            { name: 'nama_strategi', type: 'string' }
        ],
		url: "<?php echo base_url(); ?>spkp_pjas_a001/json_program",
		cache: false,
			...
			columns: [
				{ text: '#', align: 'center', filtertype: 'none', sortable: false, width: '5%', cellsrenderer: function (row) {
                     ...
                     if({add_permission}==true){
                        return "<div style='text-align: center;padding:3px'><a href='javascript:void(0);'><img border=0 src='<?php echo base_url(); ?>public/images/del.gif' onclick='dodel_program("+dataRecord.urut+","+dataRecord.id_program+")'></a>  <a href='javascript:void(0);'><img border=0 src='<?php echo base_url(); ?>public/images/edt.gif' onclick='edit_program("+dataRecord.id_program+");'></a></div>";
                     }else{
				...
				// END
                { text: 'Strategi', datafield: 'nama_strategi', columntype: 'textbox', filtertype: 'list', width: '31%' },
	   ...
	   $('#btn_tambah').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_pjas_a001/add_program" , function(response) {
		...
		$('#printbutton').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_pjas_a001/html_program",
		...
		$('#excelbutton').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_pjas_a001/excel_program",
				success: function(response){
    ...
    function edit_program(id){
        ...
        $("#popup").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_pjas_a001/edit_program/"+id , function(response) {
			$("#popup_content").html("<div>"+response+"</div>");
		});
	}

	function save_edit_program_dialog(content){
	   	$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_pjas_a001/doedit_program",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog();
    ...
    function save_add_program_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_pjas_a001/doadd_program",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog();
    ...
    function dodel_program(urut,id){
		if(confirm("Anda yakin akan menghapus data ("+urut+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/spkp_pjas_a001/dodel_program/"+id,
				success: function(response){
					 if(response=="1"){
						 $('#clearfilteringbutton').click();
                         $('#clearfilteringbutton_kegiatan').click();
