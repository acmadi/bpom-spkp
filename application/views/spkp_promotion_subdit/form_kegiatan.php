<script type="text/javascript">
    $(document).ready(function(){
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>spkp_promotion_subdit/json_kegiatan/{thn}",
		cache: false,
		...
		{		
            ...
            rendertoolbar: function (toolbar) {
				...
				thninput.change(function(){
					window.location.href="<?php echo base_url(); ?>spkp_promotion_subdit/index/1/"+thninput.val();
                });
			},
			columns: [
	   ...
	   $('#btn_tambah').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_promotion_subdit/add_kegiatan" , function(response) {
		...
		$('#printbutton').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_promotion_subdit/html_kegiatan/{thn}",
		...
		$('#excelbutton').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_promotion_subdit/excel_kegiatan/{thn}",
				success: function(response){
    ...
    function edit_kegiatan(id){
        ...
        $("#popup").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_promotion_subdit/edit_kegiatan/"+id , function(response) {
			$("#popup_content").html("<div>"+response+"</div>");
		});
	}
	...
	function save_edit_kegiatan_dialog(content){
	   	$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_promotion_subdit/doedit_kegiatan",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog();
    ...
    function save_add_kegiatan_dialog(content){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>index.php/spkp_promotion_subdit/doadd_kegiatan",
			data: content,
			success: function(response){
				 if(response=="1"){
					 close_dialog();
    ...
    function dodel_kegiatan(urut,id){
		if(confirm("Anda yakin akan menghapus data ("+urut+") ini?")){
			$.ajax({ 
				type: "POST",
				url: "<?php echo base_url()?>index.php/spkp_promotion_subdit/dodel_kegiatan/"+id,
				success: function(response){
					 if(response=="1"){
						 $('#clearfilteringbutton').click();
                         $('#clearfilteringbutton_upload').click();
