...
$(function(){
	...
	datafields:[
		...
		url: "<?php echo base_url(); ?>content/json_content/{file_id}",
		cache: false,
	...
	$("#jqxgrid").jqxGrid(
	{
		...
		columns: [
			...
			// END
			<?php if($mod=='event'){ echo "{ text: 'Tanggal', datafield: 'waktu_kegiatan', columntype: 'date', filtertype: 'date', cellsformat:'yyyy-MM-dd', width: '10%' },"; } ?>
			{ text: '<?php if($mod!='event'){ echo 'Judul'; } else{ echo 'Kegiatan'; } ?>', datafield: 'title_content', columntype: 'textbox', filtertype: 'textbox', width: '25%' },
			<?php if($mod=='event'){ echo "{ text: 'Tempat', datafield: 'headline', columntype: 'textbox', filtertype: 'textbox', width: '15%' },"; } ?>
			{ text: 'Publish', datafield: 'publish', columntype: 'list', filtertype: 'list', width: '8%'},
			{ text: '<?php if($mod!='event'){ echo 'Author'; } else{ echo 'Publisher'; } ?>', datafield: 'author', columntype: 'list', filtertype: 'list', width: '15%' },
			<?php if($mod!='event'){
			echo "{ text: 'Tanggal Buat', datafield: 'waktu', columntype: 'date', filtertype: 'date', cellsformat:'yyyy/MM/dd HH:mm:ss', width: '15%' },"; } ?>
...
function edit(file_id,id){
	window.location.href="<?php echo base_url()?>content/edit/{mod}/"+file_id+"/"+id;
}
...
function dodel(file_id,id){
	if(confirm("Anda yakin akan menghapus data ("+id+") ini?")){
		$.ajax({ 
			type: "POST",
			url: "<?php echo base_url()?>content/dodel/{mod}/"+file_id+"/"+id,
