<script>
	$(function(){
       ...
       $("#produk_informasi").addClass("active");
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>spkp_product/json_judul/",
		cache: false,
		...
		{		
			...
			columns: [
				...
				{ text: 'Tipe', datafield: 'tipe', filtertype: 'list', columntype: 'dropdownlist', width: '9%' },
                { text: 'Keterangan', datafield: 'ket_file', columntype: 'textbox', filtertype: 'textbox', width: '14%' },
	   ...
	   $('#btn_tambah_upload').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_product/add_upload" , function(response) {
		...
		$('#printbutton_upload').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_product/html_upload/",
		...
		$('#excelbutton_upload').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_product/excel_upload/",
				success: function(response){
	...
	function edit_upload(id){
		...
		$("#popup_upload").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_product/edit_upload/"+id , function(response) {
			$("#popup_content_upload").html("<div>"+response+"</div>");
		});
	}
	...
	function download(id){
		...
		$("#popup_upload").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_product/download/"+id , function(response) {
			$("#popup_content_upload").html("<div>"+response+"</div>");
		});
	}
