    ...
    $(document).ready(function(){
		...
		$("[name='btn_simpan']").click(function(){
			...
			$.ajax({ 
				...
				contentType: false,
				processData: false,
				url: "<?php echo base_url()?>spkp_counseling_subdit/do{action}_upload/{id}",
				data: data,
		...
		$("[name='btn_delete']").click(function(){
			if(confirm('Hapus dokumen ini?')){
				...
				$.ajax({ 
					...
					contentType: false,
					processData: false,
					url: "<?php echo base_url()?>spkp_counseling_subdit/delete_upload/{id}",
					success: function(response){
						res = response.split("_");
						 if(res[0]=="OK"){
							 $.notific8('Notification', {
					...
					<tr>
						...
						<td>
                            <select size="1" name="kegiatan" style="margin: 0;width: 250px;height:25px;padding:2px">
                                <?php
                                $data_kegiatan = $this->spkp_counseling_subdit_model->get_kegiatan($thn);
