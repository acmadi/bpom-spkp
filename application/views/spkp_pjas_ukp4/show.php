<script>
	$(function(){
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>spkp_pjas_ukp4/json_judul/{thn}",
		cache: false,
		...
		{		
            ...
            rendertoolbar: function (toolbar) {
				...
				thninput.change(function(){
					window.location.href="<?php echo base_url(); ?>spkp_pjas_ukp4/index/"+thninput.val();
                });
			},
			columns: [
	   ...
	   $('#btn_tambah_upload').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_pjas_ukp4/add_upload" , function(response) {
		...
		$('#printbutton_upload').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_pjas_ukp4/html_upload/{thn}",
		...
		$('#excelbutton_upload').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_pjas_ukp4/excel_upload/{thn}",
				success: function(response){
	...
	function edit_upload(id){
		...
		$("#popup_upload").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_pjas_ukp4/edit_upload/"+id , function(response) {
			$("#popup_content_upload").html("<div>"+response+"</div>");
		});
	}
	...
	function download(id){
		...
		$("#popup_upload").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_pjas_ukp4/download/"+id , function(response) {
			$("#popup_content_upload").html("<div>"+response+"</div>");
		});
	}
