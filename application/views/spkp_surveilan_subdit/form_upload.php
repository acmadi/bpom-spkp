<script>
	$(function(){
	   ...
	   var source = {
		...
		url: "<?php echo base_url(); ?>spkp_surveilan_subdit/json_upload/{thn}",
		cache: false,
		...
		{		
            ...
            rendertoolbar: function (toolbar) {
				...
				thninput.change(function(){
					window.location.href="<?php echo base_url(); ?>spkp_surveilan_subdit/index/"+thninput.val();
                });
			},
			columns: [
			    ...
			    { text: 'Kegiatan', datafield: 'nama', columntype: 'textbox', filtertype: 'textbox', width: '15%' },
	   ...
	   $('#btn_tambah_upload').click(function(){
			...
			$.get("<?php echo base_url();?>spkp_surveilan_subdit/add_upload" , function(response) {
		...
		$('#printbutton_upload').click(function () {
			...
			$.ajax({ 
				...
				url: "<?php echo base_url();?>spkp_surveilan_subdit/html_upload/{thn}",
		...
		$('#excelbutton_upload').click(function () {
			...
			$.ajax({ 
				type: "POST",
				data: datastring+'&filterscount='+k,
				url: "<?php echo base_url();?>spkp_surveilan_subdit/excel_upload/{thn}",
				success: function(response){
	...
	function edit_upload(id){
		...
		$("#popup_upload").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_surveilan_subdit/edit_upload/"+id , function(response) {
			$("#popup_content_upload").html("<div>"+response+"</div>");
		});
	}
	...
	function download(id){
		...
		$("#popup_upload").jqxWindow('open');
		$.get("<?php echo base_url();?>spkp_surveilan_subdit/download/"+id , function(response) {
			$("#popup_content_upload").html("<div>"+response+"</div>");
		});
	}
