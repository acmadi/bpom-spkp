<script type="text/javascript">
    $(document).ready(function(){
		...
		$("input[name='keterangan']").jqxInput({ theme: 'fresh', height: '22px', width: '400px' }); 
		...
		$("[name='btn_simpan']").click(function(){
			...
			$.ajax({ 
				...
				contentType: false,
				processData: false,
				url: "<?php echo base_url()?>spkp_absen_fingerprint/do{action}/{id}",
				data: data,
		...
		$("[name='btn_delete']").click(function(){
			if(confirm('Hapus dokumen ini?')){
				...
				$.ajax({ 
					...
					contentType: false,
					processData: false,
					url: "<?php echo base_url()?>spkp_absen_fingerprint/dodelete/{id}",
					success: function(response){
						res = response.split("_");
						 if(res[0]=="OK"){
							 $.notific8('Notification', {
		...
		<tr>
			<td>
				...
				<table border="0" cellpadding="3" cellspacing="2">
					<tr>
						<td colspan="3" style="font-size:11px">Pastikan format tanggal benar:<br> (YYYY/MM/DD atau DD/MM/YYY atau YYYY-MM-DD atau DD-MM-YYY)<td>
					</tr>
					<tr>
						<td>Tahun / Bulan</td>
						<td>:</td>
					...
					<tr>
						<td>Keterangan</td>
						<td>:</td>
						<td><input type="text" size="40" name="keterangan" value="<?php 
