...
class Admin_master_industri_pabrik extends CI_Controller {
	...
	function index(){
		$this->authentication->verify('admin_master_industri_pabrik','edit');
	...
	function add($id_industri){
		$this->authentication->verify('admin_master_industri_pabrik','add');

		$data['title_form']="Pabrik &raquo; Tambah";
		$data['action']="add";
		$data['id_industri'] = $id_industri;
		$data['id_plant'] = $this->generate_id($id_industri);

		$this->form_validation->set_rules('id_industri', 'Id Industri', 'trim|required|callback_kode_check');
		$this->form_validation->set_rules('id_plant', 'Id Pabrik', 'trim|required');
		...
		if($this->form_validation->run()== FALSE){
			...
			$data['content'] = $this->parser->parse("admin/industri/form_pabrik",$data,true);
			$this->template->show($data,"home");
		}elseif($this->admin_master_industri_model->insert_pabrik()){
	...
	function edit($id_industri="",$id_plant=1)
	{
		$this->authentication->verify('admin_master_industri_pabrik','edit');

		$data = $this->admin_master_industri_model->get_pabrik($id_industri,$id_plant); 
		$this->form_validation->set_rules('alamat_pabrik', 'Alamat Pabrik', 'trim|required');
		
		if($this->form_validation->run()== FALSE){
			$data['id_industri'] = $id_industri;
			$data['id_plant'] = $id_plant;
			$data['title_form']="Pabrik &raquo; Ubah";
			$data['action']="edit";
	
			$data['content'] = $this->parser->parse("admin/industri/form_pabrik",$data,true);
			$this->template->show($data,"home");
		}elseif($this->admin_master_industri_model->update_pabrik($id_industri,$id_plant)){
			$this->session->set_flashdata('alert_form', 'Save data successful...');
			redirect(base_url()."index.php/admin_master_industri/edit/$id_industri");
		}else{
			$this->session->set_flashdata('alert_form', 'Save data failed...');
			redirect(base_url()."index.php/admin_master_industri_pabrik/edit/".$id_industri."/".$id_plant);
		}
		
	}
	
	function generate_id($id){
		$qid = $this->admin_master_industri_model->get_max_pabrik($id);
		$id = intval($qid["MAX"])+1;
